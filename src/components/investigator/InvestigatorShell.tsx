...
import { Bell, Home, Languages, LogOut, Network, Brain, History, Radar, ShieldCheck } from "lucide-react";
...
const NAV = [
  ...
  { to: "/investigator", label: "Investigation Workspace", hint: "Adaptive Copilot", icon: Radar },
  { to: "/investigator/cin", label: "CIN", hint: "Criminal Intelligence Network", icon: Network },
  { to: "/investigator/bip", label: "BIP", hint: "Behavioral Intelligence Profile", icon: Brain },
  { to: "/investigator/history", label: "Investigation History", hint: "Prior conversations", icon: History },
] as const;
...
export function InvestigatorShell({ caseId, children }: Props) {
  const [lang, setLang] = useState<"EN" | "ಕನ್ನಡ">("EN");
  const navigate = useNavigate();

  return (
    <div data-workspace="investigator" className="flex min-h-screen w-full flex-col bg-background">
      <IntelligenceTicker />
      ...
      <header className="grid h-14 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-border bg-card px-4">
        <Link to="/console" className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0 leading-none">
            ...
            <div className="label-meta mt-1 truncate">WS-01 · Investigator · AIC</div>
          </div>
        </Link>
      ...
      <div className="flex flex-1 flex-col lg:flex-row">
        <aside className="shrink-0 border-b border-border bg-sidebar lg:w-56 lg:border-b-0 lg:border-r">
          <div className="hidden border-b border-border px-3 py-3 lg:block">
            <div className="label-tech">Operational Modules</div>
          </div>
          <nav className="flex overflow-x-auto lg:flex-col lg:overflow-visible lg:py-2">
            {NAV.map((item) => (
              <Link
                ...
                activeOptions={{ exact: item.to === "/investigator" }}
              ...
              >
                ...
                <span className="min-w-0">
                  ...
                  <span className="label-meta !text-[9px] hidden truncate lg:block">{item.hint}</span>
                </span>
              </Link>
            ))}
          </nav>
